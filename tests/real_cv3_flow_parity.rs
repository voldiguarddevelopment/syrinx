...
//!
//! Tolerances: pure-lookup/linear ~0; conv/mu < 1e-4; DiT `est_step1_out` and final
//! `mel` < 1e-3 (candle-vs-torch fp32 over a deep transformer).
...
fn real_cv3_flow_estimator_matches_reference() {
    ...
    assert!(d < 1e-3, "DiT est_step1_out max-abs diff {d:.3e} exceeds 1e-3");
}
...
fn real_cv3_flow_mel_matches_reference() {
    ...
    assert!(d_est < 1e-3, "est_step1_out diff {d_est:.3e} exceeds 1e-3");
    assert!(d_mel_full < 1e-3, "mel_full diff {d_mel_full:.3e} exceeds 1e-3");
    assert!(d_mel < 1e-3, "mel diff {d_mel:.3e} exceeds 1e-3");
}
