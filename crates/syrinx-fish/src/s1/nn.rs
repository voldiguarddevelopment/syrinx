//! Shared Candle primitives for the s1 (Llama-style `DualARTransformer`) backend:
//! a name-indexed weight bag, RMSNorm / linear / embedding helpers, the Fish
//! **interleaved** RoPE (`apply_rotary_emb`), the GQA self-attention block used by
//! both the slow and fast transformers, and a per-layer KV cache.
//!
//! These mirror the reference `fish_speech/models/text2semantic/llama.py` math
//! exactly. The one load-bearing difference from the CosyVoice ports in
//! `syrinx-lm/src/cv2` is the RoPE convention: Fish uses the **GPT-J interleaved**
//! pairing (`reshape(..., -1, 2)` over adjacent dims) via `precompute_freqs_cis` +
//! `apply_rotary_emb`, not the HF `rotate_half` (split-half) convention. We port
//! the interleaved form here.
...
/// reference (which casts to f32 inside RMSNorm).
pub struct Weights {
...
/// Per-attention-block geometry (shared schema for the slow + fast transformers).
#[derive(Clone, Copy)]
...
pub fn attention(
    ...
    // Optional per-head QK-RMSNorm (Qwen3-style; off for s1's Llama backbone).
    let (q, k) = if qk_norm {
