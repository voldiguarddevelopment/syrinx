...
/// Smallest weight (in elements) worth quantizing in [`Cv3Hift::load_quantized`]. Below
/// this the Q4_0 per-block scale overhead dominates; tiny params stay exact f32 (matches
/// the CV2 [`crate::real::HiftVocoder`] threshold).
...
/// [`Cv3Hift::weight`] for an f32 tensor and gets one back — only the *resident* storage is
/// the int4 `QTensor`. The original logical conv shape `[out,in,k]` is kept so `weight` can
/// restore it after dequantizing the 2-D block store.
struct QStore {
    qt: QTensor,
    shape: Vec<usize>,
}
...
impl Cv3Hift {
    ...
    /// (≥ [`QUANT_MIN_ELEMS`]) and its flattened `in·k` inner dim is 32-aligned, it is
